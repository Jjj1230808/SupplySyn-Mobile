...
<!-- 第四级页面 -->
	<view>
		<navbar style="position: fixed;left: 0;right: 0;top: 0;" :title="locationTitle" :isCanBack="true" :text="stationNo"  >
		</navbar>
		...
		<scroll-view class="material-list">
			<location-choose :projectList="projectList"  :currentRoute1="currentRoute1" :locationTitle="locationTitle" ></location-choose> 
		</scroll-view>
		<!-- <scroll-view class="material-list">
			<view class="">
				{{projectList}}
...
<script>
	...
	this.projectCode = JSON.parse((options.item)).projectCode
	this.wbs = JSON.parse((options.item)).wbs
	this.stationNo = JSON.parse((options.item)).stationNo
	
	this.currentRoute1 = options.currentRoute1

	if(this.currentRoute1 === '/pages/returnList/returnList'){
		this.locationTitle = "请选择退库单"
	}else if(this.currentRoute1 === '/pages/scrapList/scrapList') {
		this.locationTitle = "请选择报废单"
	}else if(this.currentRoute1 === '/pages/entrySign/entrySign'){
		this.locationTitle = "请选择入库签收"
	}else if(this.currentRoute1 === '/pages/scrapSign/scrapSign'){
		this.locationTitle = "请选择报废签收"
	}
	uni.showLoading({
		title: '正在查询'
	})
	let url = BaseApi + '/api/app/material/begin-assemble' ;
	let data ={
		...
		stationNo:this.stationNo
						...
						console.log(url)
							...
							uni.request({
								...
								//拿到了jsonid 发送列表请求
								 if(this.currentRoute1 === '/pages/returnList/returnList'  ){
									 this.url11 = BaseApi +'/api/app/material/return-documents/' + codeStr
								 }else if(this.currentRoute1 === '/pages/scrapList/scrapList'){
									  this.url11 = BaseApi +'/api/app/material/scrap-documents/' + codeStr
								 }else if(this.currentRoute1 === '/pages/entrySign/entrySign'){
									  this.url11 = BaseApi +'/api/app/material/inbound-signing-documents/' + codeStr
								 }else if(this.currentRoute1 === '/pages/scrapSign/scrapSign'){
									  this.url11 = BaseApi +'/api/app/material/scrap-receipt-documents/' + codeStr
								 }
								 // else if(this.currentRoute1 === '/pages/entrySign/entrySign'){
								 // 	this.locationTitle = "请选择退库签收"
								 // }else if(this.currentRoute1 === '/pages/scrapSign/scrapSign'){
								 // 	this.locationTitle = "请选择报废签收"
								 // }
			...
			console.log(options);
			this.codeStr = options.codeStr
			console.log(this.codeStr);
			let _this = this
			uni.request({
						url: BaseApi +'/api/app/material/station-no/'+ _this.codeStr  ,
						method: 'GET',
						header: {
							'Authorization': 'Bearer ' + uni.getStorageSync("scToken"),
							'Content-Type': 'application/json;charset=utf-8'
				...
				this.currentRoute1 = options.currentRoute
			if(this.currentRoute1 === '/pages/returnList/returnList'){
				this.locationTitle = "请选择退库单"
			...
			console.log(JSON.parse((options.ListData)));
			this.projectList = JSON.parse((options.ListData))
		...
		enterMaterialCode(e) {
			...
			let url = BaseApi + '/api/app/material/locating-the-station';
			let data = {
				projectCode:this.projectCode,
				wbs:this.wbs,
