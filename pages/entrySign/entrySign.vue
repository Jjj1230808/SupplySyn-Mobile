<template>
	...
	<view>
		<navbar style="position: fixed;left: 0;right: 0;top: 0;" title="入库签收" :isCanBack="true" :text="stationNo">
		</navbar>
		...
		<scroll-view class="material-list">
			...
			<material-list :materialList="materialList" @returnMaterial="returnMaterial" cardtitle="入库签收">
			</material-list>
		...
		<view class="action-buttons" v-if="!isShowSearch">
			<button class="scanSign" @click="scanAssembly"   >
				<image style="width: 36rpx;height: 36rpx;margin-right: 5rpx;" src="../../static/img/green-scan.svg">
				扫码签收
			</button>
		...
		methods: {
			...
			getfreshData() {
				if(this.saveCondition === ''){
					let _this = this;
					let url1 = BaseApi +'/api/app/material/inbound-signing-data-sources/' +_this.DocumentId;
					let url2 = BaseApi +'/api/app/material/inbound-signing-number/' + _this.DocumentId;
					...
					let _this = this;
					let url1 = BaseApi + '/api/app/material/inbound-signing/' + _this.DocumentId + '?Info=' + _this.saveCondition;
				let url2 = BaseApi +'/api/app/material/inbound-signing-number/' + _this.DocumentId;
					let request1 = new Promise((resolve, reject) => {
						uni.request({
							url: url1,
			...
			enterMaterialCode(e) {
				...
				let url = BaseApi + '/api/app/material/inbound-signing/' + this.Id + '?Info=' + e.detail.value;
				console.log(url)
				uni.request({
					url: url,
					method: 'GET',
