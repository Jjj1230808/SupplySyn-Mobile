<template>
	<view class="container">
		...
		<view class="" v-if="isShowBag">
			<view :class="[isShow ? 'showsearchBox' : 'searchBox']">
				<view class="search-container">
					...
					<view v-if="isShow" class="input-2">
						<input @tap="hideFooter1" @blur="showFooter1" type="number" v-model="POItem" placeholder="PO项次">
					</view>
					<view v-if="isShow" class="input-2">
						<input @tap="hideFooter1" @blur="showFooter1" type="number" v-model="MateNum" placeholder="物料号">
					</view>
			...
			<view class="select-print">
				...
				<view class="print-box">
					<button @click="print" class="toolbtn" :disabled="printDisable">
						<!-- <i-icon size="40rpx" color="#00893d" name="printer-fill"></i-icon> -->
									...
									<view class="arrive-num">
										...
										<view class="arriveitem">
											<input @input="checkReceiveQty($event,item)"
												@focus="focusNum($event,item.receivedQty)"
		...
		<view class="box-print" v-if="isShowBox">
			<view class="box-search">
				<view class="search-ipt">
					<view class="ipt-item">
						<input type="text" v-model="boxWBS" placeholder="WBS号" />
					</view>
					<view class="ipt-item">
						<input type="text" v-model="boxStation" placeholder="工位号" />
					</view>
					<view class="searchBtn3">
						<button @click="searchBox" type="primary">查询</button>
					</view>
				</view>
				<view class="scan-area" @click="startScanMateBox">
					<image src="../../static/scantool.svg" class="scan-area-img" mode="widthFix"></image>
				</view>
			</view>
		...
		<view class="footer-nav" v-if="showFooter">
...
<script>
	...
	var ScanDeviceClass1 = plus.android.importClass("android.device.ScanDevice");
	var scanDevice1;
	scanDevice1 = new ScanDeviceClass1();
	...
	export default {
		...
		destroyed: function() {
			console.log('页面销毁')
			scanDevice1.setOutScanMode(1); // 扫描模式=输入框
		},
		data() {
			return {
				canvasW: 200, // 画布宽
		...
		onLoad() {
			...
			scanDevice1 = new ScanDeviceClass1();
			// scanDevice1.openScan(); // 打开扫描
			scanDevice1.setOutScanMode(0); // 扫描模式
			scanDevice1.openScan()
			let _self = this;
			let _this = this
			uni.getSystemInfo({
				success: (res) => {
			...
			globalEvent.addEventListener('onPrintCallback', function(e) {
				console.log(e)
				if (e.key == 0) {
					console.log(_this.currentPrintItem)
					console.log(_this.item)
					if (_this.currentPrintItem.sizeName) {
						console.log('打印标签成功')
						console.log(_this.currentPrintItem)
						_this.printedMateOrBox('/services/app/PrintQRCode/PrintedMaterialQRCodeByMobile',
							_this.currentPrintItem)
					} else if (_this.item.id) {
						console.log('打印料盒成功')
						_this.printedMateOrBox('/services/app/PrintQRCode/PrintedMatrialBoxByMobile', _this.item)
					}
				} else if (e.key == 3) {
					uni.hideLoading()
					_this.searchMate()
					_this.isCanPrint = true
					_this.printDisable = false
					uni.showToast({
						title: '缺纸',
						icon: 'error'
					})
		...
		onHide: function() {
			console.log('scanQR隐藏')
			console.log(scanDevice1.getOutScanMode())
			uni.hideLoading()
			this.printDisable = false


			// scanDevice1.openScan(); 
			scanDevice1.setOutScanMode(1);
			scanDevice1.stopScan()
			// scanDevice1.closeScan()
			this.unregisterScan()
		},
		onBackPress(e) {
			console.log(e)
			console.log('scanQR返回')
			console.log(scanDevice1.getOutScanMode())
			// scanDevice1.openScan();
			scanDevice1.setOutScanMode(1);
			scanDevice1.stopScan()
		...
		onShow: function() {
			console.log('scanQR显示')
			scanDevice1.setOutScanMode(0);
		...
		destroyed: function() {
			console.log(scanDevice1.getOutScanMode())
			// scanDevice1.openScan(); // 打开扫描
			scanDevice1.setOutScanMode(1); // 扫描模式
		},
		...
		methods: {
			...
			searchMate() {
				...
				// this.show1 = true
				// this.error1 = '测试文字测试文字测试文字测试文字测试文字测试文字测试文字测试文字测试文字'
				uni.showLoading({
					title: '加载中',
					mask: true
				})
				...
				if (this.POItem) {
					params.poItem = this.POItem
				}
				if (this.PONum1) {
					params.poCode = this.PONum1
				}
				if (this.PackageNo1) {
					params.packageCode = this.PackageNo1
				}
				if (this.MateNum) {
					params.materialCode = this.MateNum
				}
				console.log(params)
				let api =
					`/services/app/PrintQRCode/GetBOMItems?poCode=${this.PONum1}&packageCode=${this.PackageNo1}&poItem=${this.POItem}&materialCode=${this.MateNum}`
				this.$MyHttp.get(api).then((res) => {
					...
					this.List.forEach((item, index) => {
						...
						// item.qty = index % 2 == 0 ? item.qty : Number(item.qty + '.1') test
			...
			searchBox() {
				...
				this.mateList = []
				let api =
					`/services/app/PrintQRCode/GetBoxInfoList?wbs=${this.boxWBS}&station=${this.boxStation}`
				this.$MyHttp.get(api).then((res) => {
					console.log(res)
			...
			printBagItem(item) {
				...
				console.log('生成的料袋' + id)
				console.log(item.sizeName)

				console.log(_this.currentPrintItem)

				// console.log('打印前的回纸距离', printer.getUnwindPaperLen())

				var ret = printer.initPrinter({});
				// 走纸距离20
				printer.setUnwindPaperLen(100)
			...
			print() {
				...
				this.List.forEach((item) => {
					if (item.checked == true) {
						...
						// 	this.materialInfoList.push(item)
						// } else {
						// 	this.materialInfoList.forEach((ele) => {
				...
				this.currentPrintItem = this.materialInfoList[0]
				console.log('第一个打印的', this.currentPrintItem)
				console.log('总打印列表', this.materialInfoList)

				this.getPrintItemFrameNo()

				// this.printBagItem(this.currentPrintItem)



			},
			getPrintItemFrameNo() { //打印之前还要获取该物料的建议上架位置
				let url = BaseApi +
					`/services/app/PrintQRCode/GetFrameNo?materialId=${this.currentPrintItem.bomItemId}`
				console.log(url)

				let _this = this

				if (this.currentPrintItem.size == 0) {
					uni.request({
						url: url,
						method: "GET",
						header: {
						...
						success: (res) => {
							...
							if (res.statusCode == 200) {
								_this.currentPrintItem.frameNo = res.data.result
								_this.printBagItem(_this.currentPrintItem)
								// uni.hideLoading()
							}
						},
						fail: (res) => {
					...
					_this.currentPrintItem.frameNo = ''
					_this.printBagItem(_this.currentPrintItem)
				}
			...
			printBOX(item) {
				...
				var id = uuidv4()
				this.item.id = id
				this.item.serialNo = this.item.serialNo + 1
				this.pic = ''
				this.paint()
				setTimeout(() => {
					this.disPrintBox = false
				}, 5000)
				// this.debounce(handlePrintBOX, 1500)
			...
			paint() {
				...
				setTimeout(() => {
					...
					ctx.fillText(`Station    ${_self.item.station}`, 0, 100);
					_self.toFormateStr(ctx, `Drawing  ${_self.item.drawing}`, 220, -1, 0, 130, 16)
			...
			printBoxItem(item) {
				...
				// 		console.log('打印料盒成功')
				// 		_this.printedMateOrBox('/services/app/PrintQRCode/PrintedMatrialBoxByMobile', item)
				// 	}
				// })
			},
			...
			printedMateOrBox(url, data) {
				...
				uni.request({
					...
					success: (res) => {
						console.log('-----------------打印之后接口返回------------------')
						console.log(res)
						console.log((api))
						//打印的是料袋标签且调用接口成功
						if (res.data.result.id == _this.currentPrintItem.packageId) {
							_this.currentPrintItem = {}
							console.log(_this.materialInfoList)
							_this.materialInfoList.shift()
							console.log(_this.materialInfoList)
							if (_this.materialInfoList.length > 0) {
								_this.currentPrintItem = _this.materialInfoList[0]
								console.log('下一个要打印的', _this.currentPrintItem)
						...
						} else if (res.data.result.serialNo) {
							// 打印的是料盒 则把料袋队列清空
							console.log('打印的是料盒')
							this.materialInfoList = []
							this.currentPrintItem = {}
							...
							//料盒这里必须减1
							if (this.item != {}) {
								this.item.serialNo = this.serialNo - 1
							}
							// 弹窗让用户自己确认什么情况
			...
			initScan1() {
				let _this = this;
				main1 = plus.android.runtimeMainActivity(); //获取activity  
				var IntentFilter = plus.android.importClass('android.content.IntentFilter');
				filter1 = new IntentFilter();
				filter1.addAction("scan.rcv.message"); // 换你的广播动作  
				receiver1 = plus.android.implements('io.dcloud.feature.internal.reflect.BroadcastReceiver', {
					onReceive: function(context, intent) {
						...
						// 取出taskId
						// let url =
						// `https://qakimigatewaybe.schaefflercn.com/sc/gateway/api/app/kimi-personal-scrum-board/project-action-by-task-id/${taskId}`
						// let url =
						// 	`https://kimigateway.schaefflercn.com/sc/gateway/api/app/kimi-personal-scrum-board/project-action-by-task-id/${taskId}`
						let url = BaseApi +
							`/services/app/MaterialBox/GetById?Id=${taskId}`
						if (_this.isCanScan) {
							...
							uni.request({
								...
								success: (res) => {
									...
									if (res.statusCode == 200 && res.data.result != null) {
										_this.boxWBS = res.data.result.wbs
										_this.boxStation = res.data.result.stationCode
										// uni.showToast({
										// 	icon: 'success',
										// 	title: '解析成功!'
										// })
