<template>
	<view class="mk-upload" :class="{'reverse': reverse}">
		<!--进行flex占位-->
		<block v-if="imgList.length >= column && reverse">
			<block v-if="imgItemAppendLength === 0">
				<view class="mk-upload_item" :class="uploadItemColumn"
					v-for="(item, index) in (imgItemAppendLength + (column -1))" :key="index" />
			</block>
			<block v-if="imgItemAppendLength === 1">
				<block v-if="column == 3">
					<view class="mk-upload_item" :class="uploadItemColumn" v-for="(item, index) in imgItemAppendLength"
						:key="index" />
				</block>
				<block v-if="column == 4">
					<view class="mk-upload_item" :class="uploadItemColumn"
						v-for="(item, index) in (imgItemAppendLength + 1)" :key="index" />
				</block>
				<block v-if="column == 5">
					<view class="mk-upload_item" :class="uploadItemColumn"
						v-for="(item, index) in (imgItemAppendLength + 2)" :key="index" />
				</block>
			</block>
			...
			<block v-if="imgItemAppendLength === 3">
				<block v-if="column == 5">
					<view class="mk-upload_item" :class="uploadItemColumn" :key="index" />
				</block>
			</block>
		</block>
		<!--进行flex占位-->

		<view class="mk-upload_item" :class="uploadItemColumn" v-for="(item, index) in imgList" :key="index">
			<view class="mk-upload_remove" :class="deletePosition" @tap="onDeleteThis(index)" v-if="deleteShow">
				<text>✕</text>
			</view>
			<image :src="item" class="mk-upload_item_img" @tap="onPreviewImage(index)" mode="aspectFill" />
		</view>
		<!-- 列表end -->


		<view class="mk-upload_item" :class="uploadItemColumn" v-if="controlShow && imgList.length < maxCount">
			<view class="mk-upload_add" @tap="onChooseImage">+</view>
		</view>
		<!-- 上传按钮 -->
