<template>
	<view class="htz-image-upload-list">
		<view class="htz-image-upload-Item" v-for="(item,index) in uploadLists" :key="index">
			...
			<image :src="getFileUrl(item)" @click="imgPreview(getFileUrl(item))"></image>
		...
		methods: {
			...
			imgDel(index) {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							// console.log(this.uploadLists[index].splice('/')[3]);
							let idstr = JSON.stringify(this.uploadLists[index])
							let id = idstr.split('/')[3].substring(0, 36)
							let type = idstr.split('/')[3].substring(36, 40)
							console.log(id);
							console.log(type);
							...
							uni.request({
								url: 'https://qakimigatewaybe.schaefflercn.com/sc/gateway/api/app/ali-cloud-oSS/delete-from-oss',
								method: 'POST',
								header: this.headers,
			...
			imgUpload(tempFilePaths, type) { //type 0 为图片 1为视频
				...
				tempFilePaths.forEach((item, index) => {
					uploadImgs.push(new Promise((resolve, reject) => {
						...
						let name = item.slice(-19)
						console.log(name);
						const uploadTask = uni.uploadFile({
							url: this.action, //接口地址
							filePath: item,
							name: item.slice(-19),
							fileType: 'image',
							...
							success: (uploadFileRes) => {
								...
								// let _uploadFileRes = JSON.parse(uploadFileRes.data[0])
								// _uploadFileRes[0].type = type
								...
								// console.log(_uploadFileRes);
								this.$emit("uploadSuccess", uploadFileRes.data);
								uni.hideLoading()
							},
							fail: (err) => {
								console.log(err);
